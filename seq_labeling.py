...
def attention_RNN(encoder_outputs, 
  ...
  if use_attention:
    ...
    with tf.variable_scope(scope or "attention_RNN"):
      ...
      print('shape of attention_states:', attention_states.get_shape())
      ...
      print('shape of hidden:', hidden.get_shape())
      hidden_features = []
      v = []
      ...
      for a in xrange(num_heads):
        ...
        v.append(tf.get_variable("AttnV_%d" % a,
                                             [attention_vec_size]))
      print('shape of hidden_features:', len(hidden_features), hidden_features[0].get_shape())
      def attention(query):
      ...
      print('shape of batch_attn_size:', batch_attn_size.get_shape())
      attns = [tf.zeros(batch_attn_size, dtype=dtype)
               for _ in xrange(num_heads)]
      for a in attns:  # Ensure the second shape of attention vectors is set.
        a.set_shape([None, attn_size])
      print('shape of attns:', len(attns), attns[0].get_shape())
